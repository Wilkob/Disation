...
class plant:
    ...
    def __init__(self, x, y, Name):
        ...
        self.rawresoures = 500#random.randint(0, 100)
        self.equipment = 500#self.trooppop * 3 + #random.randint(0, 100)
    ...
    def tick(self):  # changes thr planets internal levels
        ...
        if self.civpop != 0:
            ...
            #print((self.equipment , producede))
            try:
                self.TroopsSupplied = int(self.equipment / self.trooppop * 100)
                #print(self.TroopsSupplied , int(self.equipment / self.trooppop * 100),self.equipment,self.trooppop)
                Fitness += 0.5
            except:
...
def ending(run1,T,gen=None,Ai = None):  # making sure all planets have pop in them and if not close the program
    global Fitness
    #file = open("finaloutput105.txt", "a")
    totalpop = 0
    for i in planets:
        totalpop += i.givepop()
    if totalpop > 0 and run1 == True :#and T <= 2500
        run = True
    else:
...
def tofile(data):
    file = open("finalrunningoutput105.txt", "a")
    file.write(data)
    file.close()

def tick():  # has all the planet update there interal levels
    global tickdone, Fitness,planets
    if tickdone == 10:
        tickdone = 0
        #totalpop = 0
        #data = ""
        for i in planets:
            i.tick()
            #totalpop += i.givepop()
            #data = data + (str(i.givepersupply()) + ";")
        #data = data + (str(totalpop) + ";" + str(Fitness) + "\n")
        #ofile(str(data))
    else:
        tickdone += 1
    shipsmove()
