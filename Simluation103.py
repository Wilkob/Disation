...
class plant:
    ...
    def __init__(self, x, y, Name):
        ...
        self.rawresoures = 600#random.randint(0, 100)
        self.equipment = 600#self.trooppop * 3 + #random.randint(0, 100)
    ...
    def outputall(self):  # outpus all
        print("\nName:",
              ...
              "\nrawrate:", self.rawrate,
              "\nequiprate:", self.equiprate,
              "\npopraw:", self.popraw,
    ...
    def tick(self):  # changes thr planets internal levels
        ...
        if self.civpop != 0:
            ...
            if self.TroopsSupplied <= 0:
                ...
                self.trooppop = int(self.trooppop * 0.99)
                Fitness -= 0.5
...
def ending(run1,T):  # making sure all planets have pop in them and if not close the program
    global Fitness
    file = open("finaloutput103.txt", "a")
    totalpop = 0
    for i in planets:
        totalpop += i.givepop()
    if totalpop > 0 and run1 == True: #and T <= 1000
        run = True
    else:
...
def AI(net):
    for i in planets:
        ...
        try:
            ...
            print("RAWS = ", RAWS, len(RAWS))
            print("EQP =", EQP, len(EQP))
            output = net.activate((*RAWS, *EQP))
        for out in range(10):
            if output[out] > 0.5:
                if output[10] > 0.5:
